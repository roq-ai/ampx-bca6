...
function DashboardPage() {
  ...
  const chargeStationStat = useSWR<PaginatedInterface<ChargeStationInterface>>(
    () => `/charge-stations`,
    () => getChargeStations({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: chargeStationStat?.data?.totalCount,
      isLoading: chargeStationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Appointments',
