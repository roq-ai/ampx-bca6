...
import { getChargingHistoryById, updateChargingHistoryById } from 'apiSdk/charging-histories';
...
function ChargingHistoryEditPage() {
  ...
  const handleSubmit = async (values: ChargingHistoryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateChargingHistoryById(id, values);
      mutate(updated);
      resetForm();
